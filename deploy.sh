#!/bin/bash
# =============================================================================
# deploy.sh — Production VPS deployment for Planary Web
# =============================================================================
#
...
echo "Starting VPS deployment of Planary Web..."
...
read_env_var PLANARY_WEB_IMAGE
read_env_var NEXT_PUBLIC_API_BASE_URL
read_env_var NEXT_PUBLIC_SITE_URL

COMPOSE_FILE="docker-compose.vps.yml"
PROJECT="planary-web-vps"
DEPLOY_LOCK="${TMPDIR:-/tmp}/planary-web-deploy.lock"
DEFAULT_IMAGE="ghcr.io/somadina94/expenses-tracker-web:latest"
PLANARY_WEB_IMAGE="${PLANARY_WEB_IMAGE:-$DEFAULT_IMAGE}"
DEPLOY_BUILD_LOCAL="${DEPLOY_BUILD_LOCAL:-0}"
export PLANARY_WEB_IMAGE
...
stop_own_stack() {
    echo "Stopping Planary Web stack (if running)..."
...
pull_image() {
    echo "Pulling prebuilt image: ${PLANARY_WEB_IMAGE}"
    login_ghcr
    local attempt
    for attempt in 1 2 3; do
        if docker pull "${PLANARY_WEB_IMAGE}"; then
            return 0
        fi
        echo "Pull failed (attempt ${attempt}/3), retrying in 15s..."
        sleep 15
    done
    echo "Failed to pull ${PLANARY_WEB_IMAGE}"
...
build_image_local() {
    ...
    PLANARY_WEB_IMAGE="${PROJECT}-planary:latest"
    export PLANARY_WEB_IMAGE
}
...
if ! $DOCKER_COMPOSE -p "$PROJECT" -f "$COMPOSE_FILE" ps --status running | grep -q "planary"; then
...
echo "Site: ${NEXT_PUBLIC_SITE_URL:-http://localhost:5000}"
